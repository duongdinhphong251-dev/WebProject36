...
@Injectable()
export class SpaAvatarStorageService {
  ...
  async save(file: BannerUploadFile): Promise<string> {
    if (!this.storage || !this.bucketName) {
      throw new InternalServerErrorException('GCS_BUCKET_NAME is not configured for spa avatar uploads');
    }

    const extension = extname(file.originalname || '').toLowerCase() || '.bin';
    const filename = `${Date.now()}-${randomUUID()}${extension}`;
    const objectPath = `spa-avatars/${filename}`;
