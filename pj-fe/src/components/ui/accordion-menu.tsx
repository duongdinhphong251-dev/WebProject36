...
const itemVariants = cva(
  'text-foreground hover:bg-accent hover:text-accent-foreground data-[selected=true]:bg-accent data-[selected=true]:text-accent-foreground focus-visible:bg-accent focus-visible:text-accent-foreground relative flex w-full cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 text-start text-sm outline-hidden transition-colors select-none disabled:bg-transparent disabled:opacity-50 [&_a]:flex [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg]:opacity-60 [&_svg:not([class*=size-])]:size-4 [&>a]:w-full [&>a]:items-center [&>a]:gap-2',
  {
    variants: {
      variant: {
        default: '',
...
function AccordionMenuItem({
  ...
  const { classNames, selectedValue, matchPath, onItemClick } = React.use(AccordionMenuContext);
  return (
...
function AccordionMenuSubContent({
  ...
  const { nestedStates, setNestedStates, classNames } = React.use(AccordionMenuContext);
  let currentValue;
  if (type === 'multiple') {
