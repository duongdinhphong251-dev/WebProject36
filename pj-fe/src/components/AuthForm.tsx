...
export function AuthForm({ mode }: { mode: Mode }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const data = await requestJson<{ user: { role: string } }>(
        `/api/auth/${mode}`,
        'POST',
        body,
      );
  ...
  return (
    <div className="auth card stack">
      ...
      <div className="row">
        ...
        {mode === 'login' && (
          <>
            <Link href="/register">Đăng ký người dùng</Link>
            <Link href="/register/owner">Đăng ký chủ spa</Link>
          </>
        )}
      </div>
    </div>
